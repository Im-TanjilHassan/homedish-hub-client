...
const ManageRequest = () => {
  ...
  const { data: pendingAdmin = [] } = useQuery({
    queryKey: ["adminRequests"],
    enabled: dbUser?.role === "admin",
    retry: false,
    queryFn: async () => {
      const res = await axiosSecure.get("/adminRequests");
      return res.data;
    },
  });
  ...
  const handleAcceptAdmin = async (uid) => {
    ...
    if (!result.isConfirmed) return;
    try {
      const res = await axiosSecure.patch(`/adminRequests/accept/${uid}`);
  ...
  const handleRejectAdmin = async (uid) => {
    ...
    try {
      const res = await axiosSecure.patch(`/adminRequests/reject/${uid}`);
              ...
              {pendingAdmin.length ? (
                <div>
                  <table className="table">
                    ...
                    <tbody className="space-y-15">
                      {pendingAdmin.map((pendingData, index) => (
                        <RequestTable
                          key={index}
                          pendingData={pendingData}
                          handleAcceptAdmin={handleAcceptAdmin}
